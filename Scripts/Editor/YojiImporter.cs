...
namespace Yoji.Editor
{
    public class YojiImporter : AssetPostprocessor
    {
        ...
        void OnPostprocessModel(GameObject obj)
        {
            ...
            var setting =
                AssetDatabase.FindAssets("t:ConvertSetting", new[]{ assetDir })
                    .Select(x => AssetDatabase.LoadAssetAtPath<ConvertSetting>(AssetDatabase.GUIDToAssetPath(x)))
                    .FirstOrDefault();
        ...
        private void ModifyMesh(Mesh mesh, ConvertSetting setting)
        {
        ...
        private void ModifyMaterials(Material[] materials, ConvertSetting setting)
        {
...
#if false
        public ConvertSetting[] _convertSettings;
        ...
        static int IndexOfConvertSetting(IEnumerable<ConvertSetting> settings, string name)
        {
            int idx = 0;
        ...
        static ConvertSetting FindConvertSetting(IEnumerable<ConvertSetting> settings, string name)
        {
        ...
        void MakeConvertSettings(GameObject srcObj)
        {
            ...
            var newSettings = new List<ConvertSetting>();
            var materials = EnumerateMaterials(srcObj);
            foreach (var material in materials)
            {
                ...
                {
                    newSettings.Add(new ConvertSetting(material));
                    continue;
                }
